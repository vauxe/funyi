...
needs_model = pytest.mark.skipif(not MODEL, reason="set FUNYI_MLX_HYMT_MODEL to a local HY-MT checkpoint")
...
def _hymt_config_dict() -> dict:
    ...
    return {
        ...
        "rms_norm_eps": 1e-5,
        "rope_theta": 10000.0,
        "rope_scaling": {"type": "dynamic", "alpha": 1000.0, "factor": 1.0, "mscale": 1.0},
        "use_qk_norm": True,
...
def test_config_parses_nested_fields():
    ...
    assert cfg.hidden_size == 2048 and cfg.num_attention_heads == 16 and cfg.num_key_value_heads == 4
    assert cfg.head_dim == 128 and cfg.intermediate_size == 6144 and cfg.num_hidden_layers == 32
...
def test_effective_rope_base_uses_alpha():
    ...
    assert _effective_rope_base(10000.0, 128, {"type": "linear", "factor": 2.0}) == 10000.0
...
def test_end_to_end_translate_mlx():
    ...
    out = translator.translate("Hello, world.", target_language="Chinese", source_language="English")
