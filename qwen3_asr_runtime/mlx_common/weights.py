...
def quantize_predicate(weights: Dict[str, mx.array]):
    ...
    stripped = {k[len(_HF_PREFIX):] if k.startswith(_HF_PREFIX) else k for k in weights}
...
def map_and_load(weights: Dict[str, mx.array], model: nn.Module, compute_dtype: mx.Dtype) -> nn.Module:
    ...
    for key, value in weights.items():
        ...
        if target is not None and value.ndim == 4 and tuple(value.shape) != tuple(target.shape):
    ...
    if missing:
        raise KeyError(f"model params not provided by checkpoint: {sorted(missing)[:10]}")
    if extra:
        raise KeyError(f"checkpoint tensors not consumed by model: {sorted(extra)[:10]}")
    for k in model_keys:
        want = tuple(model_params[k].shape)
        got = tuple(mapped[k].shape)
        if want != got:
            raise ValueError(f"shape mismatch for {k}: model {want} vs checkpoint {got}")
