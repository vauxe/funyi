...
#
# Modified by Funyi contributors in 2026 for local runtime integration.
import math
...
class Qwen3ASRAudioEncoder(Qwen3ASRPreTrainedModel):
    ...
    def _pack_single_audio(
        ...
        padded_feature = flattened.view(chunk_count, max_chunk_len, mel_bins).transpose(
            1, 2
        )

        chunk_lengths = torch.full(
            (chunk_count,), chunk_width, dtype=torch.long, device=feature_lens.device
        )
        ...
        padded_mask_after_cnn = positions.unsqueeze(
            0
        ) < feature_lens_after_cnn.unsqueeze(1)
    ...
    def forward(
        ...
        padded_feature, padded_mask_after_cnn = self._pack_audio(
    ...
    def __init__(self, config):
        ...
        if "forced_aligner" in config.model_type:
            ...
            self.lm_head = nn.Linear(
                config.text_config.hidden_size, int(classify_num), bias=False
            )
        else:
            self.lm_head = nn.Linear(
    ...
    def get_audio_features(
        ...
        if audio_feature_lengths is None:
            if feature_attention_mask is None:
                raise ValueError(
                    "feature_attention_mask or audio_feature_lengths must be provided"
                )
