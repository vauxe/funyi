...
def _effective_rope_base(
    rope_theta: float, head_dim: int, rope_scaling: Optional[dict]
) -> float:
    if not rope_scaling:
...
class MLXHunyuanConfig:
    ...
    def from_dict(
        ...
        head_dim = int(
            raw.get("head_dim")
            or raw.get("attention_head_dim")
            or (raw["hidden_size"] // raw["num_attention_heads"])
        )
